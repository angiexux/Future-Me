...
**This repository (MVP):** the Anthropic key is read only on the server: Vite dev middleware (`vite.config.ts`) or the Node static server (`server/index.mjs`, `npm start` after `npm run build`). The browser only calls `POST /api/llm`. Optional **`VITE_API_BASE`** points the SPA at another origin that implements the same route. Static hosting alone (no proxy) cannot safely call Anthropic without exposing the key.
